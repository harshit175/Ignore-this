...
for r=1:m
    ...
    else %i.e. PDMS, has cases for a gp in the r direction, and no gp
        if gpratio~=1
            rleft=r2(c-n1);
            rright=r2(c-n1+1);
        else
...
%those elements is in PDMS (mat=2) and is a constant (equal to zero)
for i=1:totalnodes
    if (Cc(i,i)==0)
        Cc(i,i)=1;
    end
...
bb=1:n1+1;   %bb=ignition boundary nodes, modification maybe needed for conducting material
...
DT=CT-K; %equation to be solved is CT*T(n)=DT*T(n-1)+F;
...
for step=2:te
    ...
    T(:,step)=mu(i);
    alpha(:,step)=mu(j);
    Pc=Pcfunction(n1,n,m,totalnodes,T(:,step),alpha(:,step),mlump);
    %Pc(bb)=0;
    if (mean(alpha(n+2:n+2+n1,step))>0.95 && check==0)
        AB=AB0; A=A0; F=F0; 
        check=1;
