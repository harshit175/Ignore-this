...
for r=1:m
    ...
    k=stiffnessfunctiondiag(rleft,rright,zbot,ztop,mat,kc);
...
if exist('bb')
    for p=1:totalnodes
        F(p)=F(p)+Tb*( sum(B11(p,bb))-sum(A11(p,bb)) );
        %F0(p)=F0(p)+Tb*( sum(DT(p,bb))-sum(CT(p,bb)) );
    end
...
for step=2:te
    ...
    U = invAB*U+invA*sparse(vertcat(F', Pc_pre));
    end
    if not(isreal(U))
    ...
    if mod(step,recordstep)==0
        ...
        if (mean(alpha(nodesnearignition,step/recordstep))>0.99 && check==0)
            A=A0; A11=A011; B=B0; B11=B011; invA=invA0; invAB=invAB0;  F=F0; 
            check=1;
