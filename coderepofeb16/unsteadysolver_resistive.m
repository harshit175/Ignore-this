...
mu=vertcat(T(:,1), alpha(:,1));

check=0;
%Pc=zeros(totalnodes,1);
disp('time marching starting')
for step=2:te
    mu = AB*mu+A*sparse(vertcat(F', Pc));
    if not(isreal(mu))
    ...
    %T(:,step)=mu(i);
    %alpha(:,step)=mu(j);
    Pc=Pcfunction(n1,n3,n,m,totalnodes,mu(i),mu(j),mlump);
    if check==0 && (exist('bb'))
    Pc(bb)=0;
    end
    ...
    if mod(step,recordstep)==0
        T(:,step/recordstep)=mu(i);
        alpha(:,step/recordstep)=mu(j);
        if (mean(alpha(nodesnearignition,step/recordstep))>0.95 && check==0)
            AB=AB0; A=A0; F=F0;
            check=1;
